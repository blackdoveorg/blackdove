...
if ($issueResult->num_rows > 0) {
	...
	while($row = $issueResult->fetch_assoc()) {
		...
		if ($email != 'NaN' && $crossDistance < 50)
		{
			$candidate = "<h2>Candidate</h2>";
		} else {
			$candidate = '';
		}
		$rellaxValue = $fkreWeight;
		
		$issueHTML .= "<div class=\"seed rellax\" id=\"seed$id\" data-rellax-speed=\"$rellaxValue\" fkreWeight=\"$fkreWeight\"><center><b>Issue $id</b><br/>$issue ($fkreI)<br/><br/><b>Solution</b><br/>$solution ($fkreS)<br/>$foundIssueHTML$foundSolutionHTML<br/>$actionableText <b>Data</b><br/>Cross Distance: $crossDistance KM<br/>FKRE: $fkreWeight<br/>$candidate</center></div>";
