...
global $mainId;
$mainId =  $_GET['id'];
$issueSelect = "SELECT *, X(coordinate) as longitude, Y(coordinate) as latitude FROM `issues` WHERE `id` = $mainId OR `refer` = $mainId ORDER BY `datetime` DESC LIMIT 100000;";
// echo $issueSelect;
...
if ($issueResult->num_rows > 0) {
	...
	while($row = $issueResult->fetch_assoc()) {
		...
		{
			// echo "In Loop";
				global $protestWestLongitude;
				global $protestEastLongitude;
				global $protestNorthLatitude;
				global $protestSouthLatitude;
				global $mainLatitude;
				global $mainLongitude;
				$issue = preg_replace("/\r|\n/", "", nl2br(htmlspecialchars($row['issue'])));
				$solution = preg_replace("/\r|\n/", "", nl2br(htmlspecialchars($row['solution'])));
				$mainLatitude = $row['latitude'];
				$mainLongitude = $row['longitude'];
				$protestWestLongitude = $row['westLongitude'];
				$protestEastLongitude = $row['eastLongitude'];
				$protestNorthLatitude = $row['northLatitude'];
				$protestSouthLatitude = $row['southLatitude'];
		}
		$issuesInText = array();
		...
		{
			$icon = "mainIcon";
			$mainIssueHTML = "<div class=\"main\"><center><b>Issue $mainId</b><br/>$issue<br/><br/><b>Solution</b><br/>$solution<br/><br/>$foundIssueHTML<br/><br/>$foundSolutionHTML<br/></div>";
		} else
		{
			$icon = "defaultIcon";
		}
		$issueHTML .= "<div class=\"seed\" id=\"seed$id\"><center><b>Issue $id</b><br/>$issue<br/><br/><b>Solution</b><br/>$solution<br/><br/>$foundIssueHTML<br/><br/>$foundSolutionHTML</center></div>";
		$issueJS .= "var marker$id = L.marker([$latitude, $longitude], {icon: $icon}).addTo(mymap);";
...
<html>
	...
	<body>
		...
		<div class="content">
			...
			<h2>Operations</h2>
			<h3>Issue Topic</h3>
			<?php echo $mainIssueHTML; ?>
			<h3>Join a Virtual Protest</h3>
			<p><b>1.</b> Zoom into the <img src="http://www.blackdove.org/intelligence/operations/people.svg" style="width: 32px;"/> icon on the map until you can click in the red box.</p>
			<p><b>2.</b> Click anywhere in the red box.</p>
			<p><b>3.</b> Click Protest! and fill out the short form on the next page.</p>
			<h3>Map</h3>
			<div id="mapid"></div>
			<h3>Problem & Solution Relationships</h3>
			<div id="chartdiv"></div>
			<h3>Other Voices</h3>
			<div id="container">
						<?php echo $issueHTML; ?>
		...
		var mymap = L.map('mapid', {
			zoomSnap: 1
		}).setView([<?php echo "$mainLatitude, $mainLongitude"; ?>], 10);
		...
		var westLongitude = <?php echo $protestWestLongitude; ?>;
		var eastLongitude = <?php echo $protestEastLongitude; ?>;
		var northLatitude = <?php echo $protestNorthLatitude; ?>;
		var southLatitude = <?php echo $protestSouthLatitude; ?>;
		var refer = <?php echo $mainId; ?>;
		function onMapClick(e) {
			var zoomLevel = mymap.getZoom();
			var clickedLatitude = e.latlng.lat;
			var clickedLongitude = e.latlng.lng;
			
			if (clickedLongitude > westLongitude && clickedLongitude < eastLongitude && clickedLatitude > southLatitude && clickedLatitude < northLatitude)
			{
				popup
					.setLatLng(e.latlng)
					.setContent("<a href=\"http://www.blackdove.org/intelligence/report.php?refer=" + refer + "&westLongitude=" + mymap.getBounds().getWest() + "&eastLongitude=" + mymap.getBounds().getEast() + "&southLatitude=" + mymap.getBounds().getSouth() + "&northLatitude=" + mymap.getBounds().getNorth() + "&centerLatitude=" + e.latlng.lat + "&centerLongitude=" + e.latlng.lng + "\" target=\"_blank\">Protest!</a>")
					.openOn(mymap);
			} else
			{
				popup
					.setLatLng(e.latlng)
					.setContent("Try clicking in the red box.")
					.openOn(mymap);
			}
		}
		...
		var defaultIcon = L.icon({
			iconUrl: 'http://www.blackdove.org/intelligence/operations/x-men.svg',
			iconSize: [16, 16],
		});
		var mainIcon = L.icon({
			iconUrl: 'http://www.blackdove.org/intelligence/operations/people.svg',
			iconSize: [32, 32],
		});
		<?php echo $issueJS; ?>
	</script>
